...
Brewer::Constant* Brewer::Parser::ParseConstant(Type* type)
{
    if (At(IntToken))
    {
        ...
        if (!int_type)
            Error("constant int must have int type");
    ...
    if (At(FloatToken))
    {
        ...
        if (!float_type)
            Error("constant float must have float type");
    ...
    if (At(StringToken))
    {
        ...
        if (!array_type)
            Error("constant string must have array type");
        const auto size = array_type->GetNumElements();
        if (size != str.size())
            Error("constant string size must equal array size");
